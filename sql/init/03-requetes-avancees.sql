...
CREATE OR REPLACE FUNCTION upsert_adresse(
    ...
    p_id_fantoir VARCHAR(20),
    p_nom_voie VARCHAR(200),
    p_type_voie VARCHAR(50),
    p_numero VARCHAR(10),
    p_rep VARCHAR(20) DEFAULT NULL,
    p_lon DECIMAL(10, 7) DEFAULT NULL,
    p_lat DECIMAL(10, 7) DEFAULT NULL,
    p_x DECIMAL(12, 2) DEFAULT NULL,
    p_y DECIMAL(12, 2) DEFAULT NULL,
    p_type_position VARCHAR(50) DEFAULT 'entrée'
) RETURNS INTEGER AS $$
DECLARE
    ...
    v_code_departement VARCHAR(3);
BEGIN
    -- Extraire le code département (2 ou 3 caractères selon DOM-TOM)
    v_code_departement := CASE
        WHEN LEFT(p_code_insee, 3) IN ('971', '972', '973', '974', '975', '976', '977', '978', '984', '986', '987', '988', '989') THEN LEFT(p_code_insee, 3)
        WHEN LEFT(p_code_insee, 2) IN ('2A', '2B') THEN LEFT(p_code_insee, 2)
        ELSE LEFT(p_code_insee, 2)
    END;
    ...
    VALUES (
        v_code_departement,
        'Département ' || v_code_departement
    )
    ON CONFLICT (code_departement) DO NOTHING;

    SELECT id INTO v_id_departement
    FROM departement
    WHERE code_departement = v_code_departement;
    ...
    IF p_lon IS NOT NULL AND p_lat IS NOT NULL THEN
        ...
        FROM position
        WHERE lon = p_lon AND lat = p_lat AND x = p_x AND y = p_y;
    END IF;

    -- Etape 7 : Inserer ou mettre a jour l'adresse
    INSERT INTO adresse (id_ban, numero, rep, id_voie, id_position, certification_commune, date_creation, date_maj)
    VALUES (p_id_ban, p_numero, p_rep, v_id_voie, v_id_position, TRUE, NOW(), NOW())
...
-- Test de validation :
-- INSERT INTO position (x, y, lon, lat, type_position) VALUES (0, 0, 200, 50, 'entrée');  -- ERREUR : lon > 180
-- INSERT INTO position (x, y, lon, lat, type_position) VALUES (0, 0, 3.5, 100, 'entrée'); -- ERREUR : lat > 90
-- INSERT INTO code_postal (code_postal, libelle_acheminement) VALUES ('123', 'TEST'); -- ERREUR : pas 5 chiffres


-- ============================================================================
-- 3. TRIGGER : MISE A JOUR AUTOMATIQUE DE LA DATE DE MODIFICATION
-- ============================================================================

-- Fonction trigger pour gerer automatiquement la date de mise a jour
CREATE OR REPLACE FUNCTION update_date_maj()
RETURNS TRIGGER AS $$
BEGIN
    -- Lors d'un UPDATE : on met a jour date_maj
    NEW.date_maj = NOW();
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
DROP TRIGGER IF EXISTS trg_adresse_update_date ON adresse;
CREATE TRIGGER trg_adresse_update_date
    BEFORE UPDATE ON adresse
    FOR EACH ROW
    EXECUTE FUNCTION update_date_maj();

-- Test de fonctionnement :
-- UPDATE adresse SET numero = '43' WHERE id = 1;
-- SELECT id_ban, numero, date_creation, date_maj FROM adresse WHERE id = 1;
-- La date_maj sera automatiquement mise a jour avec l'heure actuelle
